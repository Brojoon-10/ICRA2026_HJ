...
**주의: `kappa_thr`과 트랙 곡률 매칭.**
TUM upstream 기본값 `kappa_thr=0.1 rad/m`은 풀사이즈 서킷(Dallara AV-21) 기준.
RC카 1/10 스케일 트랙은 곡률이 훨씬 높음 — 본 트랙(`gazebo_wall_2_iy`)의 실측:
- 최대 곡률: **0.90 rad/m**
- 평균 곡률: **0.23 rad/m**
- 트랙의 58%가 0.1 rad/m 초과

`kappa_thr=0.1`이면 코너 진입 시 **모든 55개 후보가 곡률 검증에서 탈락** → soft_check가
1개만 살림 → RViz에서 빨강(best) 하나만 보이고 나머지 전부 invalid(옅은 회색).
`kappa_thr=1.0`으로 변경 후 코너에서도 다수의 valid 후보가 생존.

**새 맵 적용 시 반드시 `kappa_thr`을 해당 트랙 최대 곡률 이상으로 설정할 것.**
확인 방법:
```python
import numpy as np
omega_z = [float(track.Omega_z_interpolator(s)) for s in track.s]
print('max |kappa|:', np.max(np.abs(omega_z)))
```
...
| **Valid (통과, 비선택)** | 검은색 (0.1, 0.1, 0.1) | 0.03m | 0.45 | 물리 검증 통과했지만 cost에서 밀림 |
