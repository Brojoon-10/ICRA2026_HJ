...
class OvertakingIYNode:
    ...
    def _prewarm_solver(self):
        ...
        obs_idx = np.arange(n // 3, 2 * n // 3, dtype=int)
        try:
            _, info = self.solver.solve(SQPProblem(
                ...
                obs_indices=obs_idx,
                obs_center_d=np.full(obs_idx.size, 0.1),
                obs_min_dist=np.full(obs_idx.size, 0.55),
    ...
    def _rolling_step(self, considered_obs, dry_run=False):
        ...
        for o in clusters:
            ...
            if o.is_static or i1 == i0:
                ...
                for ii in range(i0, i1 + 1):
                    obs_idx_list.append(ii)
                    obs_center_list.append((o.d_left + o.d_right) / 2.0)
                    obs_min_list.append((o.d_left - o.d_right) / 2.0
                                        + self.width_car + self.evasion_dist)
            else:
                fixed_center = (o.d_left + o.d_right) / 2.0
                for ii in range(i0, i1 + 1):
                    obs_idx_list.append(ii)
                    obs_center_list.append(fixed_center)
                    obs_min_list.append(self.width_car + self.evasion_dist)

        obs_idx = np.array(obs_idx_list, dtype=int)
        obs_center = np.array(obs_center_list, dtype=float)
        obs_min = np.array(obs_min_list, dtype=float)
        ...
        if obs_center.size > 0:
            oc_min = float(obs_center.min())
            oc_max = float(obs_center.max())
            oc_std = float(obs_center.std())
        else:
        ...
        diag.data = [oc_min, oc_max, oc_std,
                     float(obs_center.size), float(len(considered_obs)),
