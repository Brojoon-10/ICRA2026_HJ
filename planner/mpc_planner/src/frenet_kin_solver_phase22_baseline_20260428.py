...
class FrenetKinMPC:
    ...
    def setup(self):
        ...
        solver_opts = {
            ...
            ###      위반으로 infeas 보고. Tolerance 1cm align (사용자 룰 1cm
            ###      guard 와 부합, 어거지 X). `acceptable_*` 도 같이 명시해서
            ###      "Solved_To_Acceptable_Level" 신호도 활용 가능.
            'ipopt.tol':                       1e-4,   # KKT residual (default 1e-8 → 1e-4)
            'ipopt.constr_viol_tol':           1e-2,   # 1cm (default 1e-4 → 1e-2)
