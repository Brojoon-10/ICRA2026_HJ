...
class FrenetKinMPC:
    ...
    def _build_corridor_bounds(self, d_left_arr, d_right_arr,
        ...
        Obstacle half-planes apply ONLY to horizon steps k whose predicted
        obstacle station is within gap_long of ref_s[k]. Far-away obstacles
        must NOT clamp the initial condition (k=0) — if they did, n0 could
        jump away from the ego's actual lateral offset and the trajectory
        would start parallel to ego instead of under it.
        """
        N1 = self.N + 1
        dL = np.asarray(d_left_arr, dtype=float)
        dR = np.asarray(d_right_arr, dtype=float)
        rs = np.asarray(ref_s, dtype=float)
        ...
        if side in (SIDE_LEFT, SIDE_RIGHT) and obs_arr is not None:
            ...
            for o in range(obs_arr.shape[0]):
                ...
                s_o = obs_arr[o, :, 0]
                n_o = obs_arr[o, :, 1]
                # Per-k gating: only apply half-plane where obstacle is
                # longitudinally close to the horizon step.
                near = np.abs(rs - s_o) < self.gap_long
                if side == SIDE_LEFT:
                    contrib = np.where(near, n_o + self.gap_lat, -np.inf)
                    lb_obs = np.maximum(lb_obs, contrib)
                elif side == SIDE_RIGHT:
                    contrib = np.where(near, n_o - self.gap_lat, +np.inf)
                    ub_obs = np.minimum(ub_obs, contrib)
            if any_active:
    ...
    def solve(self, initial_state, ref_slice,
        ...
        nlb, nub = self._build_corridor_bounds(dL, dR, obstacles, side, rs)
