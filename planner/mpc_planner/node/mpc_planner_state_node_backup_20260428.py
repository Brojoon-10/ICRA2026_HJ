...
class MPCPlannerStateNode:
    ...
    def _plan_loop(self, event):
        ...
        ### HJ : 2026-04-27 — HARD RULE: obstacle in consideration horizon
        ###      MUST publish NLP-derived path (tier 0 or tier 1 HOLD_LAST).
        ###      Geometric fallback (tier 2) is FORBIDDEN with obstacle
        ###      around — would publish raceline-ish curve while obstacle
        ###      is right there. User mandate: "장애물이 horizon 내에
        ###      있을 경우 절대로 recovery / GB 경로가 나오면 안 됨".
        ###      With obstacle present, extend HOLD_LAST indefinitely
        ###      (ignore fail_streak gate). If we have no last_good_traj
        ###      either, drop through — tier-3 raceline slice is still
        ###      published as last resort, but tier-2 is skipped entirely.
        ...
        if obs_in_horizon_strict:
            ### HJ : tier-2 forbidden when obstacle is around. No last_good
            ###      either → fall through to tier-3 raceline slice (last
            ###      resort, NOT a recovery curve).
            rospy.logwarn_throttle(
                0.5,
                '[mpc][%s] obstacle in horizon + no last_good — '
                'skipping tier-2 (no recovery/GB allowed) → tier-3 raceline',
                rospy.get_name())
        ### HJ : end
