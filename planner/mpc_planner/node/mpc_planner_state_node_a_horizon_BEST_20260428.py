...
from plan_scorer import pick_plan_scored as _pick_plan_scored, horizon_aware_d_free as _horizon_d_free  # noqa: E402
...
class MPCPlannerStateNode:
    ...
    ### HJ : 2026-04-28 Stage 2 Phase 2-1 — plan-specific weight overlay.
    def _apply_plan_weights(self, plan):
        """plan 의 weight overlay 를 solver 에 push.
        _apply_mode_weights / _apply_variance_inflation / _apply_*_boost 의
        결과 위에 덮어써서 plan-specific 동작 강제.

        plan dict: {'name': str, 'side_int_match': int, 'weights': dict}
        """
        if plan is None:
            return
        try:
            ...
            if weights:
                ...
                # Force re-apply of mode_weights next tick (weight has changed).
                self._last_applied_weight_alpha = None
        except Exception as e:  # pragma: no cover
            rospy.logwarn_throttle(
    ...
    def _plan_loop(self, event):
        ...
        try:
            ...
            ### HJ : 2026-04-28 (a)+passing freeze — 진짜 근본 원인 해결.
            ###      bag 분석 (analyze_dfree_oscillation.py): ego 가 obstacle
            ###      옆 통과 시점 (|ds|<1.5m) 에 d_free 가 large swing
            ###      (정상 시 ±0.04, passing 시 ±0.5+). 즉 d_free 계산 자체가
            ###      ego≈obs 위치에서 ambiguous. 이게 plan flip 의 근원.
            ###      해결: passing 시점에 plan 변경 차단 (current 유지).
            min_obs_ds_abs = 1e9
            if obs_arr is not None and obs_arr.size > 0:
                for o in range(obs_arr.shape[0]):
                    if obs_arr[o, 0, 2] > 0:
                        # ego_s 는 ref_s[0] 와 같음. obs_s 는 obs_arr[o,0,0].
                        ds_to_obs = obs_arr[o, 0, 0] - ref_slice['ref_s'][0]
                        # wrap-aware
                        tl = self.track_length
                        if ds_to_obs > 0.5 * tl: ds_to_obs -= tl
                        elif ds_to_obs < -0.5 * tl: ds_to_obs += tl
                        if abs(ds_to_obs) < min_obs_ds_abs:
                            min_obs_ds_abs = abs(ds_to_obs)
            self._is_passing_through = (min_obs_ds_abs < 1.5)
            ### HJ : end (passing-through detect)
            ...
            try:
                hd_L, hd_R = _horizon_d_free(obs_arr, ref_slice)
                # snapshot (k=0 의 SideDecider 결과) 도 함께 사용 — average
                snap_L = float(side_scores.get('d_free_L', 0.0))
                snap_R = float(side_scores.get('d_free_R', 0.0))
                # 50/50 blend: snapshot (immediate) + horizon (future stability)
                cur_dfree_L = 0.5 * snap_L + 0.5 * hd_L
                cur_dfree_R = 0.5 * snap_R + 0.5 * hd_R
            except Exception:
                cur_dfree_L = float(side_scores.get('d_free_L', 0.0))
                cur_dfree_R = float(side_scores.get('d_free_R', 0.0))
            ema_alpha = 0.30
            prev_dfree = getattr(self, '_dfree_ema', None)
            if prev_dfree is not None:
                smooth_L = (1.0 - ema_alpha) * prev_dfree[0] + ema_alpha * cur_dfree_L
                smooth_R = (1.0 - ema_alpha) * prev_dfree[1] + ema_alpha * cur_dfree_R
            else:
                smooth_L = cur_dfree_L
                smooth_R = cur_dfree_R
            self._dfree_ema = (smooth_L, smooth_R)
            smoothed_scores = dict(side_scores)
            smoothed_scores['d_free_L'] = smooth_L
            smoothed_scores['d_free_R'] = smooth_R
            ### HJ : end (horizon + EMA)
            ...
            best_plan, all_scores, new_hist = _pick_plan_scored(
                ...
                score_gap_threshold=0.25)  # 15% → 25% (더 큰 gap 필요)
            self._plan_score_history = new_hist
            ### HJ : 2026-04-28 (a)+passing+dwell — passing-through 시 freeze.
            ###      ego 가 obs 옆 통과 (|ds|<1.5m) 시 d_free swing 으로 인한
            ###      plan flip 차단. current 유지. 통과 후 d_free 안정화 +
            ###      RACELINE 자동 전환.
            if (getattr(self, '_is_passing_through', False)
                    and current_name is not None
                    and best_plan['name'] != current_name):
                for c in candidates:
                    if c['name'] == current_name:
                        best_plan = c
                        break
            else:
                # 일반 dwell hysteresis (passing 외 시점)
                if current_name is not None and best_plan['name'] != current_name:
                    dwell = getattr(self, '_plan_dwell_count', 0)
                    if dwell < 5:
                        for c in candidates:
                            if c['name'] == current_name:
                                best_plan = c
                                break
                        self._plan_dwell_count = dwell + 1
                    else:
                        self._plan_dwell_count = 0
                else:
                    self._plan_dwell_count = 0
            ### HJ : end (passing+dwell)
        ...
        ###      cache was built for.
        ###
        ### HJ : 2026-04-27 — HARD GATE: any obstacle active in obs_arr
        ###      (within MPC consideration horizon) blocks cache hijack.
        ###      User mandate: "obstacle in horizon 이면 절대 recovery / GB
        ###      경로가 나오면 안 됨". Same gate at the failure-ladder
        ###      cache-sample below. Empty obs_arr is the only condition
        ###      that lets cache through (truly no obstacle to consider).
        _obs_in_h_pre_nlp = bool(
            obs_arr is not None
            and obs_arr.size > 0
            and (obs_arr[:, :, 2] > 0).any())
        if _obs_in_h_pre_nlp:
            cached_xy, cached_sn = None, None
        else:
        ...
        ### HJ : 2026-04-28 (S1-5) — HOLD_LAST 무한 연장 폐기 + 짧은 cap.
        ###      bag1 의 51-tick HOLD_LAST chain (3.62s, ego stuck) 가 stale
        ###      traj 발행 → inter-msg jump 1.683m → 충돌. 무한 연장 (obs_in
        ###      _horizon OR 우회) 자체가 stale 위험. 변경:
        ###        - Tier 1 HOLD_LAST: fail_streak ≤ 3 만 (yaml fail_streak_H).
        ###        - Cap 초과 + obstacle in horizon: publish 생략 (사용자 룰 1
        ###          준수). Controller latest_threshold (200ms) 가 자체 brake.
        ###        - Cap 초과 + obstacle 없음: 기존 tier 2/3 (GB 복귀) 정상.
        obs_in_horizon_strict = bool(
            obs_arr is not None
            and obs_arr.size > 0
            and (obs_arr[:, :, 2] > 0).any())

        # Tier 1: HOLD_LAST short cap only (no obs-OR bypass).
        if self._last_good_traj is not None and \
                self._fail_streak <= self._fail_tier_H:
        ...
        if obs_in_horizon_strict:
            # Cap 초과 + obstacle in horizon → publish 생략.
            self._handle_emergency_no_traj(status, solve_ms, obs_tag)
            self._debug_log(
                tier=-1, status='EMERGENCY_NO_TRAJ', ipopt_status=status,
                iter_count=iter_count, solve_ms=solve_ms, slack_max=slack_max,
                trajectory=None, u_sol=None,
                obs_arr=obs_arr, obs_tag=obs_tag, ref_slice=ref_slice,
                initial_state=initial_state, ego_s=s_cur, ego_n=ego_n,
                warm_used=warm_used,
            )
        ...
        # ### HJ : 2026-04-27 — cache-first recovery dispatch (no-obs only).
        ...
        ### HJ : 2026-04-27 — gate geometric fallback by obstacle presence.
        ###      User mandate: with obstacle in horizon, NEVER publish a
        ###      recovery/GB-shaped curve. Skip tier-2 entirely; fall
        ###      through to tier-3 raceline slice as last resort.
        if obs_in_horizon_strict:
    ...
    def _validate_publish_wpnts(self, wpnts):
        ...
        Reject if:
          ...
          - 인접 점 ds < 0.05m (near-coincident points → kappa explode)
          - x/y/vx NaN/Inf
        ...
        # inter-msg first wpnt jump (ego_s 점프 시 skip)
        es_now = float(getattr(self, 'ego_s', 0.0) or 0.0)
        es_prev = float(getattr(self, '_ego_s_at_prev_publish', es_now))
        self._ego_s_at_prev_publish = es_now
        if abs(es_now - es_prev) > 30.0:  # sim reset / lap-wrap
            return True, 'ok_ego_jumped'
        ...
        if prev_pub is not None and len(prev_pub) > 0:
            ...
            if jump > 0.5:
                return False, 'inter_msg_jump=%.3f' % jump
        return True, 'ok'
    ### HJ : end (S1-4)
