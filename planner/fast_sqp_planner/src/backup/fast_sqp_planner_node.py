...
class OvertakingIYNode:
    ...
    def __init__(self):
        ...
        from fast_sqp_planner.cfg import OvertakingIYConfig
        self._dyn_srv = DynServer(OvertakingIYConfig, self._dyn_reconfigure_cb)
        ## IY : end
    ...
    #raceline fallback — publish ego-centered GB raceline segment as
    #       OTWpntArray. Used on SQP failure in online_mode to avoid empty
    #       publishes that let state_machine's stale cache surface.
    def _publish_raceline_fallback(self, length_m: float = 10.0) -> None:
        if (self.scaled_wpnts_msg is None
                or self.scaled_wpnts is None
                or self.scaled_max_s is None):
            self._publish_empty_otwpnts()
            return
        n_src = len(self.scaled_wpnts_msg.wpnts)
        if n_src == 0 or self.scaled_delta_s is None:
            self._publish_empty_otwpnts()
            return
        # index at ego_s (wrap with modulo)
        s0 = float(self.cur_s)
        i_start = int(np.round(s0 / self.scaled_delta_s)) % n_src
        n_take = max(2, int(length_m / self.scaled_delta_s))
        msg = OTWpntArray(header=Header(stamp=rospy.Time.now(), frame_id='map'))
        out = []
        for k in range(n_take):
            src = self.scaled_wpnts_msg.wpnts[(i_start + k) % n_src]
            w = Wpnt()
            w.id = k
            w.s_m = float(src.s_m)
            w.d_m = 0.0
            w.x_m = float(src.x_m)
            w.y_m = float(src.y_m)
            w.z_m = float(getattr(src, 'z_m', 0.0))
            w.psi_rad = float(src.psi_rad)
            w.kappa_radpm = float(src.kappa_radpm)
            w.vx_mps = float(src.vx_mps)
            w.ax_mps2 = float(getattr(src, 'ax_mps2', 0.0))
            w.d_right = float(getattr(src, 'd_right', 0.0))
            w.d_left = float(getattr(src, 'd_left', 0.0))
            out.append(w)
        msg.wpnts = out
        msg.ot_side = 'trail'
        msg.ot_line = 'trail'
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if not publish_to_sm and self.prev_d is not None:
                self.abort.reset()
            # no obstacles or outside ot sector → stop publishing (SM keeps prev path)
            if not have_obs or not self.ot_section_check:
                self.prev_d = None
                self.prev_s = None
            ...
            # update latency on success only
            if solve_ok:
    ...
    def _rolling_step(self, considered_obs, dry_run=False):
        ...
        if self.prev_d is not None and self.prev_s is not None:
            ...
            d_init = np.full(len(s_av), cur_d_now)
