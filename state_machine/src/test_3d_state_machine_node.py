...
class StateMachine:
    ...
    def __init__(self, name) -> None:
        ...
        ### HJ : test-node debug topic. JSON string per state_machine tick — fields used to verify
        ### Fix 1 (entry freshness) + R3 (sustain refresh) + R4 (clear-on-exit). Toggle: ~publish_debug.
    ...
    ### HJ : R4 — "다 쓴 경로는 폐기". sustain False로 STATIC_OT 이탈 결정될 때 호출.
    ### 캐시를 빈 상태로 되돌려, 한참 뒤 새 obs 등장 시 옛 spline이 꺼내져서 "번쩍"하는 일을 차단.
    ### 재진입은 Fix 1의 freshness gate를 통해 새 publish가 들어왔을 때만 가능.
    def _clear_static_cache(self):
        self.cur_static_avoidance_wpnts.list = []
        self.cur_static_avoidance_wpnts.array = None
        self.cur_static_avoidance_wpnts.stamp = None
        self.cur_static_avoidance_wpnts.is_init = False

    ### HJ : test-node debug snapshot — emit single JSON line per state_machine tick.
    ### `rostopic echo -c /state_machine/debug` 로 Fix 1 + R3/R4 동작을 수치로 검증.
    def _publish_debug_snapshot(self):
        try:
            import json
            now = rospy.Time.now()
            src = self.static_avoidance_wpnts
            cache = self.cur_static_avoidance_wpnts

            src_stamp_age = None
            src_len = 0
            if src is not None:
                src_len = len(src.wpnts)
                if not src.header.stamp.is_zero():
                    src_stamp_age = round((now - src.header.stamp).to_sec(), 4)

            cache_stamp_age = None
            cache_s_first = None
            cache_s_last = None
            if cache is not None and cache.stamp is not None and not cache.stamp.is_zero():
                cache_stamp_age = round((now - cache.stamp).to_sec(), 4)
            if cache is not None and cache.is_init and cache.array is not None and len(cache.array) > 0:
                cache_s_first = float(cache.array[0, 2])
                cache_s_last = float(cache.array[-1, 2])

            cur_target_s = None
            targets = getattr(self.behavior_strategy, 'overtaking_targets', None) if self.behavior_strategy is not None else None
            if targets and len(targets) > 0:
                cur_target_s = float(targets[0].s_center)

            snapshot = {
                "state": self.cur_state.value if self.cur_state is not None else None,
                "static_ot_mode": bool(self.static_overtaking_mode),
                "cur_s": round(self.cur_s, 3),
                "src_len": src_len,                           # 0 이면 planner publish 빈 메시지
                "src_stamp_age_s": src_stamp_age,             # latest_threshold=0.1s 와 비교 (Fix 1)
                "cache_is_init": bool(cache.is_init) if cache is not None else False,  # R4: False면 폐기 상태
                "cache_stamp_age_s": cache_stamp_age,         # hyst(4s)/kill(10s) 비교 기준
                "cache_s_range": [cache_s_first, cache_s_last] if cache_s_first is not None else None,
                "cur_target_obs_s": (round(cur_target_s, 3) if cur_target_s is not None else None),
                "n_obs_interest": len(self.obstacles_in_interest),
            }
            self.debug_pub.publish(json.dumps(snapshot))
        except Exception as e:
            rospy.logwarn_throttle(2.0, f"[test_3d_state_machine] debug snapshot failed: {e}")
    ...
    ##################################################################
    ### HJ : Fix 1 — remove smart-static freshness bypass branch.
    ### Smart static node is not launched in 3d_headtohead.launch, but the
    ### name-based bypass was wrongly firing for the regular static cache
    ### too (both cur_static_avoidance_wpnts and cur_smart_static_avoidance_wpnts
    ### share name='static_avoidance_planner'), disabling latest_threshold.
    ### Now: single freshness path — every cache must pass latest_threshold.
    def _check_latest_wpnts(self, src_wpnts, wpnts_data: WaypointData):
        ...
        return self._check_on_spline(wpnts_data)
    ...
    def _check_overtaking_mode_sustainability(self) -> bool:
        if self.static_overtaking_mode:
            ...
            if self._check_static_avoidance_disabled():
                self._clear_static_cache()   ### HJ : R4 — bridge 진입 시 캐시 즉시 폐기
                return False
            ### HJ : R3 — sustain 진입부에서 fresh src 있으면 캐시 갱신.
            ### 없으면 옛 캐시 그대로 유지 (_check_latest_wpnts 1차/freshness 게이트가 알아서 막음).
            ### target 변경 시 새 planner output이 들어오면 자연스럽게 캐시 교체.
            self._check_latest_wpnts(self.static_avoidance_wpnts, self.cur_static_avoidance_wpnts)
            if (
                ...
                return True
            ### HJ : R4 — static sustain 실패 = "다 쓴 경로" → 캐시 폐기.
            ### 한참 뒤 새 obs 등장 시 옛 spline이 꺼내져서 "번쩍"하는 일을 원천 차단.
    ...
    def loop(self):
        ...
        ### HJ : publish per-tick debug snapshot for Fix 1 / Fix 2 verification.
        if self.publish_debug:
