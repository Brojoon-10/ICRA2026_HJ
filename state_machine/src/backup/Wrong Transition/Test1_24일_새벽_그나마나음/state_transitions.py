...
def RecoveryTransition(state_machine: StateMachine) -> Tuple[StateType, StateType]:
    ...
    # Check proximity to both GB raceline and Smart Static path
    close_to_gb = state_machine._check_close_to_raceline(0.05) * state_machine._check_close_to_raceline_heading(20)
    close_to_smart = state_machine._check_close_to_smart_static_path(0.5) * state_machine._check_close_to_smart_static_path_heading(20)
    smart_active = state_machine.smart_static_active

    # Use Smart path proximity if active, otherwise GB raceline
    close_to_raceline = close_to_smart if smart_active else close_to_gb

    if recovery_sustainability and not close_to_raceline:
        return StateType.RECOVERY, StateType.RECOVERY

    # Recovery ended - check if should return to Smart Static or GB
    if smart_active:
        wpnts_valid = state_machine._check_latest_wpnts(
            state_machine.smart_static_wpnts,
            state_machine.cur_smart_static_avoidance_wpnts)
        ...
        if wpnts_valid and close_to_smart:
            rospy.loginfo_throttle(1.0, f"[RecoveryTransition→SMART_STATIC] Recovery ended, returning to Smart path")
            return StateType.SMART_STATIC, StateType.SMART_STATIC

    # Otherwise return to GB tracking
    return GlobalTrackingTransition(state_machine, close_to_gb)
...
def ObstacleTransition(state_machine: StateMachine, close_to_raceline) -> Tuple[StateType, StateType]:
    ...
    # Smart Static not available/close/free - check GB path (but only if smart_active=False)
    ...
    if state_machine._check_overtaking_mode() or state_machine._check_static_overtaking_mode():
        ...
        # TRAILING state: prioritize Smart Static if active and path is free
        if smart_active and wpnts_valid and close_to_raceline:
            # Smart Static active, valid, and path free - use it for TRAILING
            rospy.logwarn(f"[ObstacleTransition→TRAILING+SMART_STATIC] Using Smart path (smart_active=True, path_free=True)")
            return StateType.TRAILING, StateType.SMART_STATIC
        elif close_to_raceline:
            rospy.loginfo_throttle(1.0, f"[ObstacleTransition→TRAILING] Close to raceline, using GB")
